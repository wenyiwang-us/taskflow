  ...
  */
  ...
  public:
    ...
    */
    ...
    inline void xq_init(const size_t nworkers, const size_t worker_id, const size_t nsteals, const size_t nvtm, const size_t nwaits) {
      _xq = new BoundedXQueue<Node*>(nworkers, worker_id, nsteals, nvtm, nwaits);
    }
  ...
  private:
    ...
    uint64_t _nexec_from_self {0};
    uint64_t _nexec_from_remote {0};
    uint64_t _nexec_from_executor {0};
...
BoundedXQueue<T, LogSize>::BoundedXQueue(const size_t nworkers, const size_t worker_id, const size_t nsteals, const size_t nvtm, const size_t nwaits) : 
_nworkers(nworkers), 
...
// The original XQueue push implementation
template <typename T, size_t LogSize>
TaskQueueCode BoundedXQueue<T, LogSize>::_push_rrws(T item) {
  size_t num_tries {0};
  ...
  auto& target_deque {target_worker._xq->_dequeues[_last_q]};
  while(target_deque.dequeue[target_deque.head] != nullptr) {
    num_tries++;
    if(num_tries < 25) {
      continue;
    }
  ...
  size_t mask = _last_q == 0 ? MasterDequeueMask : DequeueMask;
  target_deque.dequeue[target_deque.head] = item;
  target_deque.head = (target_deque.head + 1) & mask;
...
// request steals: steal many
template <typename T, size_t LogSize>
inline void BoundedXQueue<T, LogSize>::_request_steals() {
  
  if(_nworkers < 2) {
    return;
  }
  // I'm idle, invalidate my stale request
  _round++;

  // send request only when we are idle and we have enough nops
  if(_nops_empty == 0 || _nops_empty < _nwaits) {
    _nops_empty++;
    return;
  }
  _nops_empty = 0;
  ...
  for(size_t i = 0; i < _nvtm; i++) {
    // search for one worker to steal from other than myself
    #ifdef TF_ENABLE_STATS
    nrequests_attempted++;
    #endif // TF_ENABLE_STATS
...
// steal many dynamic (steal from MasterQueue, dynamic (relative to the current queue size))
// num_steals = 1 + (num_tasks >> [HALF, QUARTER])
template <typename T, size_t LogSize>
inline void BoundedXQueue<T, LogSize>::_handle_requests() {
  #ifdef TF_ENABLE_STATS
  nhandled_attempted++;
  #endif // TF_ENABLE_STATS
  if (MSG_REQ2ROUND(_steal_request) == _round) {
    ...
    // steal many
    // calculate MasterQueue size
    for (size_t i = 0; i < _nsteals; i++) {
      // check thief queue
      while (target_dequeue.dequeue[target_dequeue.head] != nullptr) {
        ...
        // thief's queue is full, cannot steal more; now can accept new request
        #ifdef TF_ENABLE_STATS
        nhandled_not_stolen++;
        #endif // TF_ENABLE_STATS
        _round++;
      ...
      if (_last_q_accessed > 0) {
        ...
        if (target_dequeue.dequeue[target_dequeue.tail] != nullptr) {
          uint64_t mask =
              _last_q_accessed == _nworkers ? ExecutorDequeueMask : DequeueMask;
      ...
      for (size_t qid = 1; qid < _nworkers + 1; qid++) {
        ...
        if (target_dequeue.dequeue[target_dequeue.tail] != nullptr) {
          uint64_t mask = qid == _nworkers ? ExecutorDequeueMask : DequeueMask;
          item = target_dequeue.dequeue[target_dequeue.tail];
          target_dequeue.dequeue[target_dequeue.tail] = nullptr;
          target_dequeue.tail = (target_dequeue.tail + 1) & mask;
          goto task_found;
      ...
      // no task found, increment round and return
      _round++;
      return;

      task_found:
      // push task to thief and continue for next steal
    ...
    // all steals fulfilled, increment round and return
    _round++;
  }
...
template <typename T, size_t LogSize>
T BoundedXQueue<T, LogSize>::_pop_rrws() {
    T item{nullptr};
    // pop from MasterQueue
    if (_dequeues[0].dequeue[_dequeues[0].tail] != nullptr) {
      uint64_t head_1 = (_dequeues[0].head - 1) & MasterDequeueMask;
