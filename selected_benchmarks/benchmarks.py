...
def run(target, method, thread, round):
  if method == 'xtf':
    exe = "../build/selected_benchmarks/xtf_" + target
    method = 'tf'
  else:
    exe = "../build/selected_benchmarks/bench_" + target;

  print(exe, '-m', method, '-t', thread, '-r', round);

  with open(tmp_file, "w") as ofs:
    # print stderr to the command line
    subprocess.call(
...
def main():
  ...
  parser.add_argument(
    '-c', '--category',
    help='type of plot to run',
    default='threading',
    choices=['threading', 'runtime']
  )
...
def plot_threading(args):
  ...
  for benchmark in args.benchmarks:
    for method in args.methods:
      # ax = plot.subplot(rows, cols, plot_index)
      # nthreads is converted to a list of threads, convert back to a string
      nthreads = ' '.join(str(t) for t in args.threads)
      # X is the list of the threads
      # Y is the list of runtime
      X, Y = run(benchmark, method, nthreads, args.num_rounds)
      print(X)
      print(Y)
      plot.plot(X, Y, label=method, marker='.', color='b')
      plot.legend()
      # plot_index = plot_index + 1
